...
export const useCreatePost = () => {
  ...
  const createPostMutations = useMutation({
    mutationFn: async (postData: { content: string; imageUri?: string }) => {
      ...
      if (postData.imageUri) {
        ...
        const mimiTypeMap: Record<string, string> = {
          png: "image/png",
          gif: "image/gif",
          webp: "image/webp",
          jpeg: "image/jpeg",
        ...
        const mimiType = mimiTypeMap[fileType] || "image/jpeg";
  ...
  const handleImagePicker = async (useCamera: boolean = false) => {
    ...
    if (!result.canceled) return setSelectedImage(result.assets[0].uri);
  };
  ...
  const createPost = () => {
    if (!content.trim() && !selectedImage) {
      Alert.alert(
        "Empty Post",
        "Please write something or add photo before posting",
      );
      return;
    }
    ...
    createPostMutations.mutate(postData);
  };
  ...
  return {
    ...
    setContent,
    selectedImage,
    isCreating: createPostMutations.isPending,
